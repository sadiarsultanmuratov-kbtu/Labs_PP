...
def change_direction(e): #e = event
    ...
    global dirX, dirY, game_over
    if (game_over):
        return #edit this code to reset game variables to play again

    if (e.keysym == "Up" and dirY != 1):
        dirX = 0
        dirY = -1
        
    elif (e.keysym == "Down" and dirY != -1):
        dirX = 0
        dirY = 1

    elif (e.keysym == "Left" and dirX != 1):
        dirX = -1
        dirY = 0

    elif (e.keysym == "Right" and dirX != -1):
        dirX = 1
        dirY = 0
...
def move():
    ...
    snake.x += dirX * TILE_SIZE
    snake.y += dirY * TILE_SIZE
...
def draw():
    ...
    if (game_over):
        ...
        canvas.create_text(60, 20, font = "Arial 10", text = f"Score: {score}  Level: {level}", fill = "white")
